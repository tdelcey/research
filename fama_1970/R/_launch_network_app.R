          ...
          ggplot2::aes(
          ...
          ) %>%
            {
              if (!is.null(cluster_tooltip)) {
                . + ggplot2::aes(tooltip = cluster_tooltip)
              } else {
                .
              }
            },
...
graph <- graph_with_cluster %>%
  ...
  mutate(
    nodes_label = paste0(first_author, " (", year, "), ", title) %>%
      str_to_title()
  )
...
launch_network_app(
  ...
  cluster_information = c("first_author", "title", "year", "journal", "n"),
  cluster_tooltip = NULL,
  node_id = "nodes_id",
  node_size = "n", # <- doit exister dans les noeuds
  node_tooltip = "nodes_label", # <- c’est une colonne contenant les tooltips
  color = NULL,
  layout = NULL
)

# LA SUITE: CRRER UN MOYEN D4AJUSTER LA TAILLE DES NOEUDS ET DES LIENS SELON LE RESEAU
